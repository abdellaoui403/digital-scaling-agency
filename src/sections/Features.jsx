...
const Features = () => {
  return (
    <section>
      <Element name="features">
        ...
        <div className="flex justify-center content-center text-center relative z-2 pb-20">
          <div>
          <div className="caption small-2 uppercase text-p3">
          Methodologie de travail            </div>

            <h3 className="h3 max-md:h5 max-w-640 max-lg:max-w-md mb-7 text-p4">
            Optimisez votre présence en ligne.
            </h3>
            <p className="body-1 max-lg:max-w-sm">
            Des stratégies digitales sur mesure pour booster votre activité.
            </p>
          </div>
          ...
          {features.slice(0, 2).map(({ id, icon, caption, title, text, button }) => (
  <div
    key={id}
  ...
  >
    ...
    <p className="mb-11 body-1 max-md:mb-8 max-md:body-3">{text}</p>
            ...
            {features.slice(2, 4).map(({ id, icon, caption, title, text, button }) => (
  <div
    key={id}
    className="relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7 max-md:border-2 max-md:border-s3 max-md:rounded-3xl max-md:flex-320"
  >
